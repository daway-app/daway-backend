...
const phCenterTextPlugin={id:'phCenterText',beforeDraw(chart){const opts=chart.config.options.phCenterText;if(!opts)return;const{ctx,chartArea:{width,height,left,top}}=chart;ctx.save();ctx.textAlign='center';ctx.textBaseline='middle';const cx=left+width/2,cy=top+height/2;ctx.font='700 22px Cairo, Tajawal, sans-serif';ctx.fillStyle='#0C2224';ctx.fillText(opts.value,cx,cy-10);ctx.font='600 12px Cairo, Tajawal, sans-serif';ctx.fillStyle='#86999B';ctx.fillText(opts.label,cx,cy+12);ctx.restore();}};

function initCharts(){if(!window.Chart)return;document.querySelectorAll('[data-ph-chart]').forEach(c=>{const t=c.dataset.phChart,l=JSON.parse(c.dataset.phLabels||'[]'),d=JSON.parse(c.dataset.phData||'[]'),colors=JSON.parse(c.dataset.phColors||'[]'),ctx=c.getContext('2d'),legendOff=c.dataset.phLegend==='off',centerValue=c.dataset.phCenterValue,centerLabel=c.dataset.phCenterLabel;let cfg={};if(t==='bar')cfg={type:'bar',data:{labels:l,datasets:[{data:d,backgroundColor:colors,borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#eef4f3'}},x:{grid:{display:false}}}}};else if(t==='line')cfg={type:'line',data:{labels:l,datasets:[{data:d,borderColor:'#0B8FAC',backgroundColor:'rgba(11,143,172,0.1)',fill:true,tension:.4,pointRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#eef4f3'}},x:{grid:{display:false}}}}};else if(t==='donut'){cfg={type:'doughnut',data:{labels:l,datasets:[{data:d,backgroundColor:colors,borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'72%',plugins:{legend:legendOff?{display:false}:{position:document.dir==='rtl'?'left':'right'}}}};if(centerValue){cfg.options.phCenterText={value:centerValue,label:centerLabel||''};cfg.plugins=[phCenterTextPlugin];}}new Chart(ctx,cfg);});}
